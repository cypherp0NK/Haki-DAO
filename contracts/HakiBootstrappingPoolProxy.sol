...
interface IHakiLog{
    function addLBPToLog(address _poolAddress, string memory _cid) external;
}
...
contract HakiBootstrappingPoolProxy is HakiAbstractLBP {
     ...
     */
    function createLBP(PoolConfig memory poolConfig, string memory cid) external returns (address) {
        ...
        IHakiLog(LogAddress).addLBPToLog(pool, cid);
