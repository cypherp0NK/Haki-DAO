...
contract HAKIDAO is ReentrancyGuard{
    ...
    uint public TotalHoldings;
    bool public ProposalState = true;
    bool public VotingState = false;
    bool public RewardState = false;
    ...
    function vote(uint _proposalId) external payable nonReentrant returns (uint, VoteDetails memory){
        ...
        if(msg.value == 0){
            ...
            votes[_proposalId] += 1;
            if(msg.value >= 1e15 && msg.value < 6e16){
                votes[_proposalId] += 1;
                TotalVotes += 2;
            }
            else if(msg.value >= 6e16 && msg.value < 1e18){
                votes[_proposalId] += 4;
                TotalVotes += 5;
            }
            else if(msg.value >= 1e18){
                votes[_proposalId] += 13;
                TotalVotes += 14;
            }
            if(LUFFY.balanceOf(msg.sender) > 0){
    ...
    function claimRewards(uint _proposalId, uint _qualifiersId) external nonReentrant {
        ...
        if (rewards > 0){
            HAKI.safeTransferFrom(Reserve, msg.sender, rewards);
        }
    }

    function getIdLength() external view returns(uint){
        return ids.length;
    }
